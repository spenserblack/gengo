...
static LANGUAGES_MAP: Lazy<HashMap<String, serde_yaml::Value>> = Lazy::new(|| serde_yaml::from_str(LANGUAGES).unwrap());

fn get_matchers(name: &str) -> Vec<(String, Vec<String>)> {
    LANGUAGES_MAP.iter().map(|(language_name, attrs)| {
        let map = attrs.as_mapping().unwrap();
        let matchers = map["matchers"].as_mapping().unwrap();
        let matchers = matchers.get(name).map(|matchers| matchers.as_sequence().unwrap())
            .map(|matchers| {
                matchers.iter().map(|matcher| {
                    matcher.as_str().unwrap().to_string()
                }).collect()
            }).unwrap_or_default();
...
fn test_from_extension() {
    ...
    extensions.iter().for_each(|(language_name, extensions)| {
        extensions.iter().for_each(|extension| {
            ...
            assert!(languages.contains(&language_name.as_str()), "Language::from_extension({extension}) contains {language_name}");
        });
    });
}
...
fn test_from_filename() {
    ...
    filenames.iter().for_each(|(language_name, filenames)| {
        filenames.iter().for_each(|filename| {
            ...
            assert!(languages.contains(&language_name.as_str()), "Language::from_filename({filename}) contains {language_name}");
        });
    });
}
...
fn test_from_interpreter() {
    ...
    interpreters.iter().for_each(|(language_name, interpreters)| {
        interpreters.iter().for_each(|interpreter| {
            let languages = Language::from_interpreter(interpreter);
            let languages: Vec<_> = languages.iter().map(|language| language.name()).collect();

            assert!(languages.contains(&language_name.as_str()), "Language::from_interpreter({interpreter}) contains {language_name}");
        });
    });
}
